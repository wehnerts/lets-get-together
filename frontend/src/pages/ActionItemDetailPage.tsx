            ...
            >
                <ButtonGroup variant="text" aria-label="text button group" sx={{outlineColor: "#F6E27F"}}>
                    ...
                    {editingEnabled ? detailedActionItem &&
                        <Button sx={{color: "#F6E27F"}} onClick={() => deleteActionItem(detailedActionItem.id)}>Delete
                            Item</Button> : <Button sx={{color: "#F6E27F"}} disabled={true}>Delete Item</Button>}
                    {editingEnabled ?
                        <Button sx={{color: "#F6E27F"}} type={"submit"} form={"editActionItem"}>Submit</Button> :
                        <Button disabled={true} sx={{color: "#F6E27F"}} onClick={toggleEdit}>Submit</Button>}
                    <Button sx={{color: "#F6E27F"}} onClick={toggleEdit}>{editingEnabled ? "cancel" : "Edit"}</Button>
                    <Button sx={{color: "#F6E27F"}} onClick={() => {
                        navigate(`/new-plan/${id}`)
                    }}>New Plan</Button>
                </ButtonGroup>
            </Box>
        </div>
